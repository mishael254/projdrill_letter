		...
		if (yes_or_no == 'Y') {
			...
			while (cin >> yes_no) {
				if (yes_no == 'Y') {
					...
					while (cin >> ans_yes) {
						if (ans_yes == "Yes") {
							cout << "How old ? i mean " + sisters_name << endl;
							int age;
							while (cin >> age) {
								if (age >= 110) { cout << "Are you fucking kidding me!\nPlease input age again!\n"; }
								else if (age <= 0) { cout << "Really, how can someones age be a negative? Huh!\nInput the correct age\n"; }
							...
							cout << "Okay thats wierd \n I wont even continue to help you out with the letter till you first solve that one out\nOkay?";
						}
						else { cout << "read the instructions again ,aaah nkt\nIs it Yes or No!!?"; }
					}
